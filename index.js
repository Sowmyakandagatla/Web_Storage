...
//local storage is a windows object method. syntax of local storage(localStorage.setItem(key, value);)
const saveToLocalStorage = () =>
  localStorage.setItem("taskyCA", JSON.stringify({ card: globalTaskData }));
//inserAdjacentHTML is to insert the HTML element 
...
const editCard = (event) => {
  ...
  if (elementType === "BUTTON") {
    //  (event.target.parentNode.parentNode.parentNode) these are the no of clases from target button to its parent class in html .
    parentElement = event.target.parentNode.parentNode;
  } else {
    parentElement = event.target.parentNode.parentNode.parentNode;
  }

//parentElement.childNodes[3].childNodes[3] this to edit the current element (first writre parentElement.childNodes[3] and console it click on edit button u will find node list in that take the index u need for this element, similarlly do for another childnode.)
